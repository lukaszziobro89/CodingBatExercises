    ...
    */
    ...
    public boolean isEverywhere(int[] nums, int val) {
        ...
        for (int i = 0; i < nums.length - 1; i ++) {
            if (nums[i] != val && nums[i+1] != val ) isTrue = false;
        }
        return isTrue;
    }

    /**
     Given an array of ints, return true if the array contains two 7's next to each other, or there are two
     7's separated by one element, such as with {7, 1, 7}.
         has77([1, 7, 7]) --> true
         has77([1, 7, 1, 7]) --> true
    ...
    public boolean has77(int[] nums) {
        for (int i = 0; i < nums.length - 1; i++) {
            if (nums[i] ==7 && nums[i + 1] == 7){
                return true;
            }else if (i <= nums.length - 1 && nums[i] == 7 && nums[i + 2] == 7){
                return true;
            }
        }
        return false;
    ...
    public boolean haveThree(int[] nums) {
        int sum = 0;
        for (int i = 0; i < nums.length; i++) {
            if (i< nums.length -1 && nums[i] == 3 && nums[i+1] != 3) sum += 3;
            if (i< nums.length -1 && nums[i] == 3 && nums[i+1] == 3) return false;
            if (i == nums.length - 1 && nums[i] == 3) sum +=3;
        }
        return sum == 9;
    }
    ...
    public int[] notAlone(int[] nums, int val) {
        int[] output = new int[nums.length];
        for (int i = 0; i < nums.length; i++) {
            if (i > 0 && i < nums.length - 1 && val != nums[i-1] && val != nums[i+1]){
                output[i] = Math.max(nums[i-1], nums[i+1]);
            }else {
                output[i] = nums[i];
            }
        }
    ...
    public int[] zeroMax(int[] nums) {
        ...
        for (int i = 0; i < output.length; i++) {
            if (nums[i] == 0) {
                int[] currentRest = Arrays.copyOfRange(nums, i, nums.length);
                int largestOdd = largestOdd(currentRest);
                output[i] = largestOdd;
            }else {
                output[i] = nums[i];
            }
    ...
    private int largestOdd(int[] inputArray){
        int largestOdd = 0;
        for (int i = 0; i < inputArray.length; i++) {
            if (inputArray[i] % 2 != 0 && largestOdd < inputArray[i]) largestOdd = inputArray[i];
        }
        return largestOdd;
    }
    ...
    public boolean more14(int[] nums) {
        ...
        return noOf1s > noOf4s;
    }

    /**
     Given a number n, create and return a new string array of length n, containing the strings "0", "1" "2" .. through n-1.
     N may be 0, in which case just return a length 0 array. Note: String.valueOf(xxx) will make the
    ...
    public boolean either24(int[] nums) {
        boolean is2and2 = false;
        boolean is4and4 = false;
        for (int i = 0; i < nums.length - 1; i++) {
            if (nums[i] == 2 && nums[i+1] == 2) is2and2 = true;
            if (nums[i] == 4 && nums[i+1] == 4) is4and4 = true;
        }
        return is2and2 ^ is4and4;
    }
    ...
    public boolean has12(int[] nums) {
        ...
        for (int i = 0; i < nums.length; i++) {
            if (nums[i] == 2) indexOf2 = i;
        }
        return firstIndexOf1 < indexOf2;
    }
    ...
    public boolean twoTwo(int[] nums) {
        // check for index=1 to index = 2nd last element
        // return false if one of 2s is not followed by another 2
        for (int i = 1; i < nums.length - 1; i++) {
            if (nums[i] == 2 && nums[i-1] != 2 && nums[i+1] != 2) return false;
        }
        // if last is 2 then check if 2nd last is 2
        if (nums[nums.length - 1] ==2) return nums[nums.length - 2] == 2;
        return true;
    }
