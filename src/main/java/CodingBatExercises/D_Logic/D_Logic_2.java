...
public class D_Logic_2 {
    ...
    public boolean makeBricks(int small, int big, int goal) {
        return ((goal <= 5 * big + small) // combination of big + small might be exact or higher then goal
                &&                        // if combination is higher then we can add small ones
                (goal % 5 <= small));     // but only if length of small bricks (after using big ones) are
    }
    ...
    public boolean evenlySpaced(int a, int b, int c) {
        if(a == b && b == c){
            ...
            return ((Math.abs(a - b) == Math.abs(b - c)) || (Math.abs(a - c) == Math.abs(a - b)) || (Math.abs(c-a) == Math.abs(c-b)));
        }
    }
    ...
    public boolean closeFar(int a, int b, int c) {
        return  !isClose(b,c) & (isClose(a,b) ^ isClose(a,c));
    }
    ...
    public int makeChocolate(int small, int big, int goal) {
        int numOfBig = goal / (big*5);
        int restAfterBig = goal - numOfBig * 5;
        if (small + big * 5 < goal){
            return -1;
        } else if (restAfterBig <= 0 ){
            return 0;
        }else {
            return restAfterBig;
        }
